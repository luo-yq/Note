...
SN	方法以及描述
1	def get: A返回选项的值
2	def isEmpty: Boolean如果该选项为None返回true，否则为false
3	def productArity: Int该产品的尺寸大小。对于一个产品A(x_1, ..., x_k), 返回k
4	def productElement(n: Int): Any该产品的第n个元素，0为基础。换言之，对于一个产品A(x_1, ..., x_k),，返回 x_(n+1)，其中 0 < n < k。
5	def exists(p: (A) => Boolean): Boolean返回true，如果此选项非空，当应用于此选项的值谓词p返回true。否则返回false。
...
9	def foreach[U](f: (A) => U): Unit应用过程f给到该选项的值，如果它不为空。否则，什么都不做。
10	def getOrElse[B >: A](default: => B): B返回选项的值，如果选择不为空，否则返回默认计算的结果。
11	def isDefined: Boolean如果该选项实例为一些返回true，否则为false。
...
15	def orNull如果它不为空返回该选项的值，如果它是空则返回null。
